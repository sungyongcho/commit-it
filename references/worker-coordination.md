...
User designation grants ordinary issue administration in that repository: create and
triage issues; edit titles, bodies, scope and acceptance checks; add/edit explanatory
comments; attach supplied images; manage existing labels and priorities; reconcile
duplicates; and close/reopen issues with an evidenced reason. Do not request approval
again for each routine action. Preserve human intent, original evidence, active owners
and truthful completion states. Workflow readiness labels still require valid evidence.

Keep issue work in issues/comments. Use supported direct attachments or existing durable
attachments. Do not create Git assets, commits, commit messages, evidence-only PRs or
review tasks solely for issue messages/images. If uploading is unavailable, report the
tool limitation and retain prepared images; do not ask for already-granted permission
again or substitute Git publication. Separately requested repository assets/docs keep
their own delivery scope. Existing historical evidence remains preserved.

This grant does not authorize product implementation, Git merging, deployment, credentials,
irreversible issue deletion or checkout synchronization. Keep held drafts in the chat or
handoff surface. A user-designated single-worker epic stays whole. Added scope on ready
work normally becomes a follow-up; an explicit scope revision requires readiness reconciliation.
