...
- The coordinator handles issue intake/scope, assigns work and alone performs
...
into every mirror. Comments outside the marked record are historical evidence,
not the current assignment or an invitation to take over.
...
4. **Ready:** after scope and required checks are complete and writers have stopped,
   ...
   Include the reviewed head and reusable evidence in `Self-review: LGTM`.
   Readiness allows review of the committed head, not edits by a different worker.
5. **Revise:** before new implementation, return to Draft and `OCCUPIED`, update
   mirrors, then edit. An existing review applies only to its recorded head. Use
   `Review: LGTM` for another worker's completed work or a concrete change request.
...
do not claim synchronization succeeded. The PR's current state wins over old mirrors.
Only the owner/coordinator updates state; a reviewer reports findings without taking
over the branch. GitHub's Draft stage prevents merging, not concurrent file edits.
